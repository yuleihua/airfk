...
func TestNotifications(t *testing.T) {
	...
	if err := server.RegisterName("book", service); err != nil {
		t.Fatalf("unable to register test service %v", err)
	}

	clientConn, serverConn := net.Pipe()

	go server.ServeCodec(codec.NewJSONCodec(serverConn), OptionMethodInvocation|OptionSubscriptions)
	...
	var subid string
	response := codec.JsonSuccessResponse{Result: subid}
	...
	for i := 0; i < n; i++ {
		var notification codec.JsonNotification
...
func waitForMessages(t *testing.T, in *json.Decoder, successes chan<- codec.JsonSuccessResponse,
	failures chan<- codec.JsonErrResponse, notifications chan<- codec.JsonNotification, errors chan<- error) {
	...
	for {
		...
		for _, msg := range responses {
			...
			if _, found := msg["result"]; found {
				successes <- codec.JsonSuccessResponse{
			...
			if _, found := msg["error"]; found {
				...
				failures <- codec.JsonErrResponse{
					Version: msg["jsonrpc"].(string),
					Id:      msg["id"],
					Error:   codec.JsonError{int(params["subscription"].(float64)), params["message"].(string), params["data"]},
				}
				continue
			}
			if _, found := msg["params"]; found {
				params := msg["params"].(map[string]interface{})
				notifications <- codec.JsonNotification{
					Version: msg["jsonrpc"].(string),
					Method:  msg["method"].(string),
					Params:  codec.JsonSubscription{params["subscription"].(string), params["result"]},
				}
				continue
			}
...
func TestSubscriptionMultipleNamespaces(t *testing.T) {
	var (
		...
		out           = json.NewEncoder(clientConn)
		in            = json.NewDecoder(clientConn)
		successes     = make(chan codec.JsonSuccessResponse)
		failures      = make(chan codec.JsonErrResponse)
		notifications = make(chan codec.JsonNotification)
